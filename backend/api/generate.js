...
חלץ תובנות, לא נתונים יבשים: אל תצטט מספרים או מגמות (אסור לכתוב "הציון הוא 1", או "הגרף עולה"). תרגם את המספרים להתנהגות.
...
סגנון כתיבה ועיצוב:
כתוב בשפה יומיומית, עניינית וישירה. המטרה היא להעביר מסר נקי למפקד.

ללא כוכביות (Bullets): אל תשתמש בסימני רשימה (כמו * או -). השתמש בכותרת מודגשת (Bold) לתחילת כל משפט ואחריה טקסט זורם.

לא שפת משאבי אנוש: הימנע מביטויים מסורבלים (כמו "הצוער מפגין", "אינטראקציה", "פוטנציאל ללמידה").

לא מטאפורות מוגזמות: אל תשתמש במילים כמו "בולדוזר", "דורסני", "אריה", או "ראש בקיר".

ניסוח ישיר: "משתפר בפתרון בעיות", "מתמקד במשימה על חשבון האנשים", "מתקשה להציב גבולות כשיש ויכוח".
...
🟢 לשימור:
כותרת מודגשת של החוזקה: הסבר קצר וענייני איך זה בא לידי ביטוי בשטח ובהתנהגות (משפט אחד).
כותרת מודגשת של חוזקה נוספת (אם יש): הסבר קצר (משפט אחד).

🔴 לשיפור:
כותרת מודגשת של הפער: תיאור ענייני המבוסס על הצלבת המדדים וההקשרים (למשל: "מתקשה לשמור על סמכות והצבת גבולות כשיש מולו חייל שמתווכח").
כותרת מודגשת של פער נוסף: תיאור ענייני נוסף של הבעיה ההתנהגותית.

🛠️ המלצות להמשך:
יעד פרקטי או שאלת מיקוד: משימה ברורה להמשך או שאלה פתוחה שהמפק"צ צריך לשאול בשיחה כדי להציף את הפער (למשל: "איך לדעתך היית יכול להציב גבול ברור יותר לחייל שסירב פקודה, בלי לאבד את הסמכות?").
יעד או שאלה נוספת: משפט אחד נוסף.`
},
